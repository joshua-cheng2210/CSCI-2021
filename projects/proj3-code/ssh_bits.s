...
# mul_64_floatscale64:
#     movl %edi, %esi
#     andl $0x80000000, %esi # %esi will now store the sign of the float number
#     movl %edi, %r8d
#     andl $0x7F800000, %r8d # %r8d will now store the exponent of the float number
#     movl %edi, %r9d
#     andl $0x007FFFFF, %r9d # %r9d will now store the mantissa of the float number
# 
...
# 
# ret_floatscale64:
#     andl $0x80000000, %esi # sign
#     andl $0x7F800000, %r8d # exponent
#     andl $0x007FFFFF, %r9d # mantissa
#     orl %r8d, %r9d
#     orl %esi, %r9d
#     movl %r9d, %eax
#     ret
...
# initialize exponent
    ...
    cmpl $255, %eax # 255 compare exponent
    jne PassNan # if they're not equal; continue
    movl %edi, %eax # else uf = eax and uf is returned. 
    jmp Fin # then return

    # another else if statement 
    PassNan:
    # initalize exponent again
    movl %edi, %eax
    shrl $23, %eax
    andl $255, %eax
    ...
    cmpl $255, %edx # 255 compare exponent
    jl PassInf # edx is not 255 (inf)
...
PassInf:
    ...
    movl %edi, %eax # uf = eax
    shrl $23, %eax # uf >> 23 
    andl $255, %eax # (uf >> 23) & 255

    cmpl $0, %eax # 0 compare exponent
    je MantCheck # 0 == exponent
    addl $6, %eax # add 6 to exponent
    shll $23, %eax # exponent << 2

    # initalizing sign 
    movl %edi, %ecx # uf = ecx 
    shrl $31, %ecx # uf >> 31
    shll $31, %ecx # uf << 31
    xorl %ecx, %eax # combines sign and exponent
    movl %edi, %ecx #
    shll $9, %ecx 
    shrl $9, %ecx 
    xorl %ecx, %eax
