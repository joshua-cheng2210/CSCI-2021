...
int main(int argc, char *argv[]) {
    ...
    char *file_chars = mmap(NULL, size, PROT_READ, MAP_PRIVATE, fd, 0); // creates pointer to file contents
    ...
    char *block1 = mmap(address, block_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);

    // start new block at end of previous block, similar options as before
    char *block2 = mmap(address + block_size, block_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
